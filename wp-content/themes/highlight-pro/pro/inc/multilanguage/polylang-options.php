...
function mesmerize_multilanguage_copy_from_source()
{
    ...
    <script>
        function mesmerize_multilanguage_copy_from_source() {
            ...
            } else if (typeof edInsertContent !== "undefined" && typeof edCanvas  !== "undefined"){
                edInsertContent(edCanvas, content);
            } else {
                jQuery(".wp-editor-area").html(content);
            }

            if (wp.data && wp.data.dispatch( 'core/editor' ) && wp.data.dispatch( 'core/editor' ).hasOwnProperty("resetBlocks")) {
...
//        $result .= pll_the_languages($hidden_args);
        $result = "<div class='mesmerize-language-switcher {$ulClass}'>{$result}</div>";
    }
